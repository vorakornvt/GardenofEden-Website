...
module.exports = {
      ...
      if (snapshot.empty) {
        ...
        snapshot.forEach((doc) => {
          docs.push({
            ...
            availableOnline: doc.data().availableOnline,
  ...
  // [1B] GET availableOnline Products
  async getAvailableOnlineProducts(req, res, next) {
    try {
      // Store the collection reference in variable & call GET method (with comp index)
      const productRef = db.collection("products");
      const snapshot = await productRef
        .where("availableOnline", "==", "true") // Filtering based on availableOnline instead of onSale
        .orderBy("name", "asc")
        .limit(10)
        .get();
      ...
      if (snapshot.empty) {
        ...
        snapshot.forEach((doc) => {
          docs.push({
            ...
            sizes: doc.data().sizes,
            texture: doc.data().texture,
            availableOnline: doc.data().availableOnline, // Changing to availableOnline
            isAvailable: doc.data().isAvailable,
            image: doc.data().image,
          });
        });
  ...
  // [1B] GET Products by availableOnline, isAvailable, category, and color
  async getFilteredProducts(req, res, next) {
    try {
      ...
      // Add filters based on the query parameters if provided
      if (color) {
        productRef = productRef.where("color", "==", color);
      }
      if (category) {
        productRef = productRef.where("category", "==", category);
      }
      productRef = productRef
        .where("availableOnline", "==", "true") // it is string Filtering based on availableOnline
        .where("isAvailable", "==", "true") // it is string  Filtering based on isAvailable
        .orderBy("name", "asc")
        .limit(10);
  ...
  async postProduct(req, res, next) {
    ...
    try {
      ...
      const response = await productRef.add({
        ...
        availableOnline: req.body.availableOnline,
        image: downloadURL,
      });
